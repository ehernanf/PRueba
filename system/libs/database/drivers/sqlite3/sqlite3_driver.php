...
class CI_DB_sqlite3_driver extends CI_DB {
     ...
     */
    function db_connect() {
        return new SQLite3($this->database, SQLITE3_OPEN_READWRITE);
    }
     ...
     * @return	object
     */
    function _execute($sql) { //echo $sql;
        $sql = $this->_prep_query($sql);
        $result_id = $this->conn_id->prepare($sql);
        $result_id = $result_id->execute();
        if (is_object($result_id)) {
            if (is_numeric(stripos($sql, 'SELECT'))) {
                $this->affect_rows = $this->conn_id->changes();
            } else {
                $this->affect_rows = $this->conn_id->changes();
            }
        } else {
    ...
    function insert_id($name = NULL) {
        ...
        return $this->conn_id->lastInsertRowID($name);
    }
    ...
    function count_all($table = '') {
        ...
        if (!($query->numColumns() && $query->columnType(0) != SQLITE3_NULL)) {
            return 0;
        }
    ...
    function _error_message() {
        $error_array = $this->conn_id->lastErrorMsg();
